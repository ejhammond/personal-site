...
import { AspectImage, Message } from 'theme-ui';
...
The actual implementation of this `getNextState` function will depend on what form the `action` argument takes, so let's put the reducer on hold for a moment while we talk about _actions_.
...
<Message variant="error">The reducer must not mutate the `currentState`</Message>
