...
<Message variant="error">
  An action must be a plain JavaScript
  object that has a `type` field
</Message>
...
<Message variant="error">
  The reducer must not mutate the
  `currentState`
</Message>
...
<script>
  ...
  addButtonElement.addEventListener(
    ...
    () => {
      ...
      const ADD_1_ACTION = createAddAction(
        1,
      );
  ...
  subtractButtonElement.addEventListener(
    'click',
    () => {
      const SUBTRACT_1_ACTION = createSubtractAction(
        1,
      );
